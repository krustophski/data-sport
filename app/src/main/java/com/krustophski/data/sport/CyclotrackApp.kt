...
class CyclotrackApp : Application(), Configuration.Provider {
    ...
    private fun logAvailableSensorsOnce() {
        val prefs = PreferenceManager.getDefaultSharedPreferences(this)
        val key = "pref_available_sensors_logged"
        if (prefs.getBoolean(key, false)) {
            return
        }
        val sensorManager = getSystemService(Context.SENSOR_SERVICE) as SensorManager
        val sensors = sensorManager.getSensorList(Sensor.TYPE_ALL)
        Log.d("Sensors", "Available sensors count=${sensors.size}")
        sensors.forEach { sensor ->
            Log.d(
                "Sensors",
                "${sensor.name} | type=${sensor.type} (${sensor.stringType}) | " +
                    "vendor=${sensor.vendor} | version=${sensor.version} | " +
                    "power=${sensor.power}mA | resolution=${sensor.resolution} | " +
