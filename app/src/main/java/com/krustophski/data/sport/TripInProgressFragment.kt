...
import com.krustophski.data.sport.util.getBrightnessPreference
import com.krustophski.data.sport.util.getSafeZoneMargins
...
class TripInProgressFragment :
    ...
    private var autoBackgroundScheduled = false
    ...
    private fun scheduleAutoBackground() {
        if (autoBackgroundScheduled) return
        autoBackgroundScheduled = true
            ...
            when (val tripId = viewModel.tripId) {
                null -> {
                    ...
                    if (!autoStartIssued) {
                        ...
                        pauseButton.post { startTripListener.onClick(pauseButton) }
                    }
                }
