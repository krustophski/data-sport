...
class SensorLiveData @Inject constructor(@ApplicationContext context: Context) :
    ...
    private val magneticListener = MagneticFieldEventListener()
    ...
    inner class MagneticFieldEventListener : SensorEventListener {
        override fun onSensorChanged(event: SensorEvent) {
            Log.v(
                "SENSOR",
                "${event.sensor.name}: ${event.values[0]},${event.values[1]},${event.values[2]}"
            ...
            value = value?.copy(magneticField = event) ?: SensorModel(
                accelerometer = null,
                accelerometerAverage = null,
                gravity = null,
                gyroscopeAverage = null,
