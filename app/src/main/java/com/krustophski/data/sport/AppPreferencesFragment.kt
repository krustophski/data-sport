...
import com.krustophski.data.sport.events.GoogleFitAccessGranted
import com.krustophski.data.sport.util.configureGoogleFit
import com.krustophski.data.sport.util.deauthorizeStrava
import com.krustophski.data.sport.util.getPreferences
import com.krustophski.data.sport.util.hasFitnessPermissions
import com.krustophski.data.sport.util.updateStravaAuthToken
...
class AppPreferencesFragment : PreferenceFragmentCompat() {
    ...
    private var clearPrefsTapCount = 0
    private var clearPrefsLastTap = 0L
    ...
    private fun configureClearPreferences() {
        findPreference<Preference>(getString(R.string.preferences_clear_preferences_key))?.apply {
            ...
            onPreferenceClickListener = Preference.OnPreferenceClickListener {
                ...
                if (now - clearPrefsLastTap > 2000L) {
                    clearPrefsTapCount = 0
                }
                clearPrefsLastTap = now
                clearPrefsTapCount += 1
                val remaining = 6 - clearPrefsTapCount
                if (remaining > 0) {
                    Toast.makeText(
                        context,
                        "Tap $remaining more times to clear preferences",
                        Toast.LENGTH_SHORT
                    ).show()
